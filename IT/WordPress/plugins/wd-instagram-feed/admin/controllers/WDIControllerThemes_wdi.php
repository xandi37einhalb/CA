...
class WDIControllerThemes_wdi {
  ...
  private function save_feed(){
    require_once WDI_DIR . "/admin/models/WDIModelThemes_wdi.php";
    $model = new WDIModelThemes_wdi();
    $settings = ($_POST[WDI_TSN]);
    
    $defaults = $model->get_theme_defaults();
    ...
    if($action==0){
        $wpdb->insert($wpdb->prefix. WDI_THEME_TABLE, $settings,$this->data_format);
        if($wpdb->insert_id == false){
        $this->message(__("Cannot Write on database","wdi"),'error');
      }
    }else{
        $msg = $wpdb->update($wpdb->prefix. WDI_THEME_TABLE, $settings, array('id'=>$action), $this->data_format,array('%d'));
        if($msg == false){
  ...
  private function reset_changes(){
    ...
    $action = $_POST['add_or_edit'];
    if($action != 0){
      $reset_tab = isset($_POST['wdi_refresh_tab'])? $_POST['wdi_refresh_tab'] : '';
      $reset_section = isset($_POST['wdi_refresh_section']) ? $_POST['wdi_refresh_section'] : '';
      # getting proper tab and section option for resetting
      # then resetting options
      $form_elements = $this->view->getFormElements();
      $form_elements = $form_elements['elements'];
      $current_settings = WDILibrary::objectToArray($model->get_theme_row($action));
      foreach ($form_elements as $key => $value) {
        if(isset($value['attr'])){
          $correct_tab = false;
          $correct_section = false;
          foreach ($value['attr'] as $attr) {
            if(isset($attr['name']) && $attr['name'] == 'tab' && isset($attr['value']) && $attr['value'] == $reset_tab){
              $correct_tab = true;
            }
            if(isset($attr['name']) && $attr['name'] == 'section' && isset($attr['value']) && $attr['value'] == $reset_section){
              $correct_section = true;
            }
          }
          if($correct_tab == true && $correct_section == true){
    ...
    if($model->check_default($action) == '1'){
      $defaults['default_theme'] = '1';
    }
    global $wpdb;

    if($action==0){
        $wpdb->insert($wpdb->prefix. WDI_THEME_TABLE, $defaults,$this->data_format);
        if($wpdb->insert_id == false){
          $this->message(__("Cannot Write on database","wdi"),'error');
          $this->display();
        }else{
          $this->message(__("Successfully reseted","wdi"),'updated');
          $this->edit($wpdb->insert_id);
        }
    }else{
        unset($current_settings['id']);
        $msg = $wpdb->update($wpdb->prefix. WDI_THEME_TABLE, $current_settings, array('id'=>$action), $this->data_format,array('%d'));
        if($msg == false){
          $this->message(__("You have not made new changes in this section","wdi"),'notice');
          $this->edit();
        }else{
          $this->message(__("Section successfully reseted","wdi"),'updated');
          $this->edit();
        }
  ...
  //   $theme_id = WDILibrary::get('current_id', 0);
  //   $new_theme_id = $this->duplicate_tabels($theme_id);
  //   require_once WDI_DIR . "/admin/models/WDIModelThemes_wdi.php";
  //   $model = new WDIModelThemes_wdi();
  //   require_once WDI_DIR . "/admin/views/WDIViewThemes_wdi.php";
  //   $view = new WDIViewThemes_wdi($model);
  //   echo WDILibrary::message('Item Succesfully Duplicated.', 'updated');
  //   $view->edit($new_theme_id);
  // }
