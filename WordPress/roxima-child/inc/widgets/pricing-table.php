<?php
	class CI_Widget_Pricing_Table_CA extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			foreach ( $options as $option ) {
				...
				if ( ! empty( $option['title'] ) ) {
					?><p class="item-title"><?php echo esc_html( $option['title'] ); ?></p><?php
				}
				...
				if ( ! empty( $option['button_text'] ) && ! empty( $option['button_url'] ) ) {
					...
					<div class="item-pricing-action">
						<a href="<?php echo esc_url( $option['button_url'] ); ?>" target="_blank" class="btn"><?php echo esc_html( $option['button_text'] ); ?></a>
					</div>
					<?php
				}
	...
	register_widget( 'CI_Widget_Pricing_Table_CA' );
